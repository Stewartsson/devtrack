...
export async function fetchPublicGists(username: string, token?: string): Promise<number> {
...
export async function fetchPublicTopRepos(username: string, token?: string, days = 30): Promise<TopRepo[]> {
  const since = new Date();
  since.setDate(since.getDate() - days);
  const sinceStr = since.toISOString().slice(0, 10);
  const res = await ghFetch(`${GITHUB_API}/search/commits?q=author:${username}+author-date:>=${sinceStr}&per_page=100&sort=author-date&order=desc`, token);
  if (!res.ok) return [];
  const data = (await res.json()) as { items: Array<{ repository: { full_name: string; html_url: string } }> };
...
export async function fetchPublicContributions(username: string, token?: string, days = 30): Promise<ContributionData> {
  const since = new Date();
  since.setDate(since.getDate() - days);
  const sinceStr = since.toISOString().slice(0, 10);
  const res = await ghFetch(`${GITHUB_API}/search/commits?q=author:${username}+author-date:>=${sinceStr}&per_page=100&sort=author-date&order=desc`, token);
  if (!res.ok) return { days, total: 0, data: {} };
  const data = (await res.json()) as { total_count: number; items: Array<{ commit: { author: { date: string } } }> };
...
export async function fetchPublicStreak(username: string, token?: string, timezone?: string): Promise<StreakData> {
  const since = new Date();
  ...
  for (const item of data.items) {
    try {
      ...
      const tzDate = new Intl.DateTimeFormat("en-CA", { timeZone: tz, year: "numeric", month: "2-digit", day: "2-digit" }).format(d);
  ...
  return { current: result.current, longest: result.longest, lastCommitDate: result.lastCommitDate, totalActiveDays: result.totalActiveDays };
}

export async function fetchPublicTopLanguages(username: string, token?: string): Promise<PublicLanguage[]> {
  const res = await ghFetch(`${GITHUB_API}/users/${username}/repos?sort=updated&per_page=30`, token);
  if (!res.ok) return [];
...
export async function fetchPublicPullRequests(username: string, token?: string): Promise<number> {
  const res = await ghFetch(`${GITHUB_API}/search/issues?q=type:pr+author:${username}&per_page=1`, token);
  if (!res.ok) return 0;
  const data = (await res.json()) as { total_count?: number };
  return data.total_count ?? 0;
}

async function fetchPublicWeeklyGoalProgress(userId: string, showOnProfile: boolean): Promise<WeeklyGoalProgress | null> {
  if (!showOnProfile) return null;
  try {
    const { data: goals, error } = await supabaseAdmin.from("goals").select("current, target").eq("user_id", userId).eq("recurrence", "weekly");
...
export async function fetchPublicProfile(
  ...
  const [
    ...
    fetchPublicGists(user.github_login ?? username, githubToken),
    fetchPublicTopRepos(user.github_login ?? username, githubToken, 30),
    fetchPublicContributions(user.github_login ?? username, githubToken, 30),
    fetchPublicStreak(user.github_login ?? username, githubToken, user.timezone),
    fetchPublicTopLanguages(user.github_login ?? username, githubToken),
    fetchPublicPullRequests(user.github_login ?? username, githubToken),
    (options.includeAchievements && user.github_login)
    ...
    fetchPinnedRepoDetails(user.github_login ?? username, user.pinned_repos || [], githubToken || ""),
  ...
  try {
    const { data: widgetsRow } = await supabaseAdmin.from("users").select("public_widgets").eq("id", user.id).single();
  ...
  return {
    username: user.github_login ?? username,
    bio: user.bio ?? null,
